---
description: Compute Flesch-Kincaid grade level for a draft and flag out-of-band scores. Mechanical check, no LLM cost.
allowed-tools: Read, Bash, Glob, AskUserQuestion
argument-hint: "[draft-file-or-inline-text]"
---

You are dispatching the counterbalance readability reviewer (a pure Node function, no LLM). Follow these steps exactly.
...
## Step 3: Invoke the readability CLI

With profile:
```bash
node "${CLAUDE_PLUGIN_ROOT}/lib/readability.mjs" --file="$DRAFT_PATH" --voice-profile-file="$PROFILE_FILE"
```

Without profile:
```bash
node "${CLAUDE_PLUGIN_ROOT}/lib/readability.mjs" --file="$DRAFT_PATH"
```
...
Flesch-Kincaid grade falls within the 9–13 target. No findings.
```

If `findings` is non-empty (always exactly one finding for readability), print:

```markdown
### Readability — out of band

**Line 1** — `note` — _readability-out-of-band_
