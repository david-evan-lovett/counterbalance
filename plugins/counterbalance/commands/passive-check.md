---
description: Flag passive-voice constructions via to-be verb + past participle matching. Mechanical check, no LLM cost.
allowed-tools: Read, Bash, Glob, AskUserQuestion
argument-hint: "[draft-file-or-inline-text]"
---

You are dispatching the counterbalance passive reviewer (a pure Node function, no LLM). Follow these steps exactly.
...
## Step 3: Invoke the passive CLI

With profile:
```bash
node "${CLAUDE_PLUGIN_ROOT}/lib/passive.mjs" --file="$DRAFT_PATH" --voice-profile-file="$PROFILE_FILE"
```

Without profile:
```bash
node "${CLAUDE_PLUGIN_ROOT}/lib/passive.mjs" --file="$DRAFT_PATH"
```
...
No passive constructions flagged.
```

If `findings` is non-empty, print:

```markdown
### Passive voice — N findings
```

Iterate findings and render each as:

```markdown
**Line <line>** — `note` — _passive-voice_
