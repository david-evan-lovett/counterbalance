---
description: Flag non-stopword overuse within any 5-sentence window. Mechanical check, no LLM cost.
allowed-tools: Read, Bash, Glob, AskUserQuestion
argument-hint: "[draft-file-or-inline-text]"
---

You are dispatching the counterbalance repetition reviewer (a pure Node function, no LLM). Follow these steps exactly.
...
## Step 3: Invoke the repetition CLI

With profile:
```bash
node "${CLAUDE_PLUGIN_ROOT}/lib/repetition.mjs" --file="$DRAFT_PATH" --voice-profile-file="$PROFILE_FILE"
```

Without profile:
```bash
node "${CLAUDE_PLUGIN_ROOT}/lib/repetition.mjs" --file="$DRAFT_PATH"
```
...
No repeated words beyond threshold in any 5-sentence window.
```

If `findings` is non-empty, print:

```markdown
### Repetition — N findings
```
